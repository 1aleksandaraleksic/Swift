...
class ViewController: UIViewController {
    ...
    @IBAction func onClick(_ sender: Any) {
        ...
        let number: String = inputNumber.text!
        
        if let convertNumber:Int = Int(number) {
            let numberWords = convertNumberToWords(convertNumber)
            outputLabel.text = numberWords
        }else{
            showAlert("Morate uneti Broj")
        }
    ...
    func showAlert(_ message: String){
        ...
        alert.addAction(UIAlertAction(title: "U redu", style: .cancel))
